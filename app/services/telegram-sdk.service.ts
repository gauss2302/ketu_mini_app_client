...
export class TelegramSDKService {
	...
	public async initialize(): Promise<void> {
		...
		try {
			...
			this._error = null;
			console.log("Telegram SDK initialized successfully with initData:", this._initDataRaw);

			// Send to backend
			await this.sendToBackend();
		} catch (error) {
			this._error = error instanceof Error ? error : new Error("Unknown initialization error");
	...
	public async sendToBackend(): Promise<void> {
		...
		console.log("InitData available for authentication:", this._initDataRaw ? "Yes" : "No");
	}
